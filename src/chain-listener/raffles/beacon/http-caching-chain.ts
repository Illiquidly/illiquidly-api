...
class HttpChain implements Chain {
  ...
  async info(): Promise<ChainInfo> {
    ...
    if (
      !!this.options.chainVerificationParams &&
      !isValidInfo(chainInfo, this.options.chainVerificationParams)
    ) {
      throw Error(
        `The chain info retrieved from ${this.baseUrl} did not match the verification params!`,
      );
    }
    return chainInfo;
  }
