...
export async function fetchBeaconByTime(
  client: ChainClient,
  time: number,
): Promise<RandomnessBeacon> {
...
async function validatedBeacon(
  client: ChainClient,
...
export type RandomnessBeacon = ChainedBeacon;
...
export function isChainedBeacon(value: any, info: ChainInfo): value is ChainedBeacon {
  return (
    info.schemeID === "pedersen-bls-chained" &&
...
export function isUnchainedBeacon(value: any, info: ChainInfo): value is UnchainedBeacon {
  return (
    info.schemeID === "pedersen-bls-unchained" &&
    !!value.randomness &&
    !!value.signature &&
